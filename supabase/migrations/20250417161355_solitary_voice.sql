/*
  # Implement Sequential Lesson Locking
  ...
  1. Changes
    ...
    - Update progress trigger to ensure lessons unlock in strict sequence
...
BEGIN
  ...
  FOR v_course IN (SELECT DISTINCT course_id FROM lessons) LOOP
    FOR v_user IN (SELECT DISTINCT user_id FROM progress) LOOP
      -- Reset the last completed position for this course and user
